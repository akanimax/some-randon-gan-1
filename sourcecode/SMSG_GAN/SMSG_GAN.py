...
class Generator(th.nn.Module):
    ...
    def __init__(self, depth=7, latent_size=512, use_spectral_norm=True):
        ...
        from SMSG_GAN.CustomLayers import GenGeneralConvBlock, GenInitialBlock
    ...
    def get_gammas(self):
        """ getter for obtain the gamma values of self-attention layers"""
        gammas = []  # initialize to empty list
        for module in self.layers:
            gammas.append(module.self_attention.gamma.item())
    ...
    def forward(self, x):
        ...
        outputs, attentions = [], []  # initialize to empty lists
...
class Discriminator(th.nn.Module):
    ...
    def __init__(self, depth=7, feature_size=512, use_spectral_norm=True):
        ...
        from SMSG_GAN.CustomLayers import DisGeneralConvBlock, DisFinalBlock
        ...
        self.rgb_to_features[self.depth - 1] = \
            from_rgb(self.feature_size // np.power(2, i - 2) if i >= 2
